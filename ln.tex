...
%   流程明确且预测性强是算法的一个重要特征，即给定相同的输入，无论何时运行算法，
%   输出都应始终相同。 
%   算法可以用于执行计算、数据处理、自动推理等任务，是计算机科学中的基本概念。  
%   算法是一个有限指令序列, 用于解决某个问题.
% \end{definition}
  
% 就像很多算法有朴素的方法,往往是直观的, 但低下的效率使其无法处理大规模问题.
...
% 主要因为我们讨论的算法都是要实际应用于具体数字计算机的. 这一点在今后的讨论中不再强调.
...
% 在本章中，我们从一个非常简单的概念：List 出发，所谓 List，就是一个元素序列，序列中每个元素都用完全一样的类型，或者说，
% 数据结构。这些元素是有序的，我们可以认为，每一个元素，都有唯一的下标，或者说地址。最常见的地址形式，可能就是自然数下标。
...
所以我们这里将类声明放在 \verb|ArrayList02.h|, 将实现放在 \verb|ArrayList02.cpp| 中。然后用 \verb|main02.cpp| 来测试这个类。
...
这里注意一下类的成员函数的实现。成员函数的实现必须在函数名前加上类名和作用域解析符 \verb|::|。 这是 C++ 的语法要求。
另外，成员函数的实现可以直接访问类的 \verb|private| 成员。比如上面的 \verb|insert| 函数中，我们直接访问了 \verb|size| 和 \verb|data|。

一旦一个类被声明，我们就可以像一个普通类型那样使用它。比如上面的 \verb|main02.cpp| 中，我们创建了一个 \verb|ArrayList| 的实例 \verb|A|。
但必须在任何使用的文件中用 \verb|#include "ArrayList02.h"| 来包含类的声明。这就导致一个问题，如果有两个文件同时使用了 \verb|ArrayList02.h|,
然后这两个文件又互相发生了包含，就会导致重复定义错误。比如我们设计了一个排序类使用了这个 \verb|ArrayList| 类，我们又设计了一个搜索类也使用了这个 \verb|ArrayList| 类，
然后排序类中也可能使用了搜索类，这就导致重复定义。我们注意到系统自带的头文件，比如 \verb|<iostream>|, 似乎能很好的解决这个问题。
实际上，这个问题可以通过预处理指令来解决。我们在头文件的开头和结尾加上如下代码：
\begin{lstlisting}[language=C++]
#ifndef __CRAZYFISH_ARRAYLIST__
#define __CRAZYFISH_ARRAYLIST__
// 头文件内容
...
#endif
\end{lstlisting}
这样就能确保头文件只被包含一次。这个技巧是 C/C++ 编程中非常常用的。
...
在我们的例子中，列表其实是有足够的复杂性，因此建议显式地定义构造函数。同时因为存在动态内存分配，
我们必须显式地定义析构函数，确保内存被正确释放。
我们下面将更新后的设计 \verb|ArrayList03.h| 和 \verb|ArrayList03.cpp|，
以及测试 程序 \verb|main03.cpp| 列出如下：
\begin{lstlisting}[language=C++]
// ArrayList03.h
#ifndef __CRAZYFISH_ARRAYLIST__
#define __CRAZYFISH_ARRAYLIST__
...
以上代码尽管简单，已经包含了一些我们推荐的设计原则。比如：
\begin{itemize}
  \item 类的声明和实现分开，分别放在头文件和源文件，但构造函数和析构函数放在头文件中；
  \item 头文件使用预处理指令防止重复包含；
  \item 数据成员使用默认初始化；
  \item 对于复杂的类，显式定义构造函数和析构函数；
  \item 析构函数确保动态分配的内存被正确释放；
\end{itemize}
...
我们将上面的语法全部纳入我们的设计，得到最终的设计 \verb|ArrayList04.h| 和 \verb|ArrayList04.cpp|, 以及测试程序 \verb|main04.cpp| 如下：
\begin{lstlisting}[language=C++]
// ArrayList04.h
#ifndef __CRAZYFISH_ARRAYLIST__
#define __CRAZYFISH_ARRAYLIST__
...
public:
  ...
  void printList() const;  // 列出表内全部元素.
  void insert(t_ele _val, t_idx _pos); // 在指定位置插入指定元素值.
};
#else
/// DO NOTHING.
#endif
...
%     \verb|NULL|. \verb|NULL| 其实就是一个 \verb|#define NULL 0| 的宏,
%     而 \verb|nullptr| 是一个类型安全的指针空值. 
% \end{itemize}
...
% 这里我们注意到第一个指针的应用是传递了 \verb|name|. 字符串作为一个数组，我们用首地址传递。这是 C 语言的传统。但这样的传递是不安全的。
% 因为我们可以通过这个指针修改 \verb|name| 的内容。
% 这是一个潜在的 bug。
...
public:
  ...
  ArrayList(std::initializer_list<t_ele> list)
  {
    size = list.size();
    data = new t_ele[size];
    t_idx i = 0;
...
int main()
{
  ...
  C.printList();
  B.printList(); // B 不受影响
  ArrayList D {1, 2, 3, 4, 5};
  D.printList();
  return 0; 
}
\end{lstlisting}

我们丰富一下我们的列表的功能，同时讨论一个设计原则，不要冗余。注意到这里的 \verb|makeEmpty| 和析构函数有功能的重叠。
这种情况下，我们应该让析构函数调用 \verb|makeEmpty|, 而不是两个函数都实现同样的功能。这样可以避免冗余代码，减少错误。
类似的情况，还有 \verb|find| 和 \verb|remove|, 这里 \verb|remove| 调用了 \verb|find|, 也是为了避免冗余代码。
...
这里注意到模板类和模板函数的配合使用。模板类的成员函数也是模板函数，因此在实现时也需要加上 \verb|template <typename T>|，
并且在函数名前加上 \verb|SingleLinkedList<T>::| 来确保模板参数的一致性。
...
注意到我们这里的析构函数其实是错误的，因为它没有释放节点的内存。我们来实现它：
...
这里我们发现两端代码冗余，首先不论拷贝构造还是赋值，都有复制新链表的过程；其次，赋值还包含了释放原有内存的过程，
而这个和析构函数的功能是一样的。为了避免冗余，我们可以考虑构建两个内部函数，一个是释放内存，一个是复制链表。
...
我们将这两个函数设计成私有成员函数，因为它们不需要被外部调用。然后我们实现它们：
...
然后结合这两个函数，我们可以实现一个外部接口：
\begin{lstlisting}
  void insertAfter(const T &_posVal, const T &_val); // 在值为 _posVal 的节点之后插入新节点
\end{lstlisting}
实现如下：
\begin{lstlisting}
template <typename T>
...
这种内部函数和外部接口的设计，是一种常见的设计模式。它将复杂的操作分解成简单的步骤，
并通过内部函数隐藏实现细节。

现在我们来设计一个单链表中比较麻烦的过程， 删除节点。删除节点需要找到要删除节点的前一个节点，然后将前一个节点的 \verb|next| 指针指向要删除节点的下一个节点。
这就需要一个搜索前驱节点的函数：
\begin{lstlisting}
  Node* findPrev(const T &_val) const; // 查找值为 _val 的节点的前驱节点
\end{lstlisting}
实现如下：
\begin{lstlisting}
template <typename T>
...
\end{lstlisting}
这个函数也应该设置成私有成员函数。这里我们注意到 \verb|findPrev| 和 \verb|find| 函数有很多相似的地方，
几乎就是一个函数的变形。我们可以考虑将它们合并成一个函数，通过参数来决定是查找前驱节点还是当前节点。
但这里我们为了代码的清晰性，暂时不做这个优化。其实，这个办法有更好的解决方案，比如总是空出一个头节点，
这样就不需要区分头节点的前驱节点和其他节点的前驱节点了。但这里我们不做这个优化。因为后续有更好的办法。
但是，这里也提醒我们，有时不要一味追求代码的简洁性，而忽略了代码的可读性；或者一味追求细节功能，而忽略了从整体设计上消除问题。
一个不好的设计，往往带来大量的细节问题。
 
然后我们可以实现一个外部接口：
\begin{lstlisting}
  void removeByValue(const T &_val); // 删除值为 _val 的节点
\end{lstlisting}
实现如下：
\begin{lstlisting}
template <typename T>
...
对于插入，我们再考虑不同的场景。
我们将插入和删除分成两个不同的操作，一个是插入到当前节点之后 \verb|push_front|，一个是插入到链表头部, \verb|push_back| 。
以及对称的操作 \verb|pop_front| 和 \verb|pop_back|。
\begin{lstlisting}[language=c++]
...
\end{lstlisting}
我们注意到之前的内部操作 \verb|insert| 对 \verb|push_front| 来说其实是多余的，因为头部不存在前驱节点。
这也是有的方案建议总是空出一个头节点的原因。
...
\end{lstlisting}
对 \verb|push_back| 也一样几乎需要重构，因为我们只记录了 \verb|tail|, 并没有记录 \verb|tail| 的前驱节点。
所以体会一下，为什么单链表其实比双链表更难实现。
...
% 列表初始化几乎不需要做任何修改。接下去是 \verb|push_front| 和 \verb|push_back|:
% \begin{lstlisting}
% template <typename T>
% void SingleLinkedList<T>::push_front(const T &_val)
% {
...
% 对称地，\verb|pop_front| 和 \verb|pop_back| 也需要修改：
% \begin{lstlisting}
% template <typename T>
% void SingleLinkedList<T>::pop_front()
% {
...
%     Node *temp = head;
%     head = head->next;
%     if (head == nullptr) // 如果链表变空，更新 tail
%         tail = nullptr;
%     delete temp;
% }
...
有了 \verb|find_prev| 函数，我们就可以实现插入、删除函数了。首先是插入函数：
\begin{lstlisting}[language=c++]
bool insert(const T &target, const T &val);
\end{lstlisting}
它的实现思路是，首先调用 \verb|find_prev| 函数找到目标节点的前驱节点。如果前驱节点为 \verb|nullptr|，则说明目标节点是头节点，
或者链表为空。如果链表为空，则插入失败，返回 \verb|false|。如果目标节点是头节点，则调用 \verb|push_front| 函数插入新节点，返回 \verb|true|。
否则，创建一个新节点，将其插入到前驱节点之后，返回 \verb|true|。
\begin{lstlisting}[language=c++]
template <typename T>
...
\end{lstlisting}
它的实现思路是，首先调用 \verb|find_prev| 函数找到目标节点的前驱节点。如果前驱节点为 \verb|nullptr|，则说明目标节点是头节点，
或者链表为空。如果链表为空，则删除失败，返回 \verb|false|。如果目标节点是头节点，则将头指针指向下一个节点，删除头节点，返回 \verb|true|。
否则，将前驱节点的 \verb|next| 指针指向目标节点的下一个节点，删除目标节点，返回 \verb|true|。
\begin{lstlisting}[language=c++]
template <typename T>
...
% 图 4.28 展示了拷贝构造函数：它遵循常规流程，先把 root 初始化为 nullptr，然后生成 rhs 的一份拷贝。我们使用一个非常巧妙的递归函数 clone 来完成所有繁琐的工作。
% % 和自己的小测试.

% 上节课基本介绍了 BST 的思想，和保持 BST 良好性质的关键：保持平衡。
% 我们现在先考虑如何设计和构建 BST。

% 首先是节点，我们参考课本的设计，定义一个简单的节点结构：
...
% 和链表一样，我们将其封装在类 BinarySearchTree 中。保持必要的安全性和便利性。

% \begin{lstlisting}[language=C++]
% /// 数据类型必须是 Comparable, 所以就用这个作为抽象模板参数.
% template <typename Comparable>
% class BinarySearchTree
% {
% public:
...
% 给定一个加权图 \(G=(V, E)\) 和一个特殊顶点 \(s\) 作为输入，找到从 \(s\) 到 \(G\) 中所有其他顶点的最短加权路径。

% 例如，在图9.8中的图中，从 \(v_1\) 到 \(v_6\) 的最短加权路径成本为 $6$, 
% 路径从 \(v_1\) 到 \(v_4\) 到 \(v_7\) 到 \(v_6\). 这些顶点之间的最短未加权路径长度为 $2$. 
% 通常，如果没有指定我们是指加权路径还是未加权路径，如果图是有权重的，则路径是有权重的。还要注意，在这张图中，
% 从 \(v_6\) 到 \(v_1\) 没有路径。

%前一个例子中的图没有负成本的边。从 \(v_5\) 到 \(v_4\) 的路径有
...
% 和 LCS 问题一样, 我们先只考虑求最短路径长度 $\delta(s, i)$,
% $i \in V$, $i \neq s$. 这样可以使解唯一.
