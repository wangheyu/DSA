...
任何概念上的抽象数据结构(ADTs)，我们都需要解决两个基本设计：
\begin{itemize}
  \item 数据的类型；
  \item 允许的操作。
\end{itemize}
...
尽管 ADTs 设计的理想是完善，但是我们也要避免因为追求完美而影响设计的实施。因此，
追求理想的同时，也要注意可行性，不要期待第一次就写出完美的设计和代码。
如果某个语法点你暂时还没有搞清楚，那么就先用你能理解的方式实现它，等到后续学习中再回来完善它。

在这份设计中，\verb|class| 关键字后面跟着的是类名 \verb|ArrayList|, 
说明我们需要设计的是一个名为 \verb|ArrayList| 的类（即 ADT ）。注意到设计的 \verb|ArrayList| 类中既包含了数据成员，也包含了操作成员。
这个和我们的 ADTs 的设计原则是一致的。

而 \verb|private| 和 \verb|public| 关键字, 则分别规定了数据成员和操作成员的可访问性。
数据成员 \verb|size| 和 \verb|data| 是私有的, 只能被类内部的成员函数访问. 
而操作成员则是共有的, 可以被类内和外部的函数访问. 这也是面向对象的设计原则. 它引出的一个设计逻辑被称为封装性，
一个完美的封装性可以理解为，设计 ADT 数据结构的目的是为用户提供一种数据类型，比如这里的 \verb|ArrayList|,
实际上就是一种功能更加完善的数组。当用户使用它时，我们应该严格区分，哪些数据和操作时允许用户访问的，哪些不允许。
并分别用 \verb|private| 和 \verb|public| 关键字来加以区分。

这一点听上去似乎很技术，但其实是自然的。我们设计一个 ADT，其实是创造一个概念。比如 \verb|ArrayList| 就是数组这个概念。
我们可以类比上帝创造一个概念，比如鸡，他会创造一个鸡的概念，然后规定鸡的属性和行为。比如鸡有颜色、体重、性别等属性，
鸡会叫、会吃、会跑等行为。这里依据这个概念创建的具体的一只鸡，如果我们希望改变它的体重，在真实的世界中，我们不能直接修改它的体重数值，
我们只能通过公共接口，比如喂它吃东西，或者让它拼命运动来实现。

所以大家考虑数据结构之所以要区分 \verb|private| 和 \verb|public|, 根本原因就是避免到处是上帝，特别是每一个用户都是上帝。这样的世界肯定不稳定，
这样的软件也是一样。当然，所有的事情都有相对性。什么是本质 \verb|private| 的，什么是本质 \verb|public| 的，这个界限并不是绝对的，
根据不同的应用场景和设计师，都会不一样。比如即便是鸡，一种极端的操作是狠狠咬它一口，那么即便不算直接修改属性，也差不多了。

正如世界上不存在鸡，因为鸡是一个抽象的概念。任何具体的一只鸡，都是这个概念的具象化，在面向对象的设计中，我们把这个概念的具象化称为实例 (instance)。

比如 \verb|A| 是 一个 \verb|ArrayList| 的实例:
\begin{verbatim}
ArrayList A;
A.insert(34, 0);
A.insert(12, 1);
...
为了将一个抽象的概念具象化，我们必须说明，它是如何被创造的。正如一个类的声明当中，必须要有一个构造函数 (constructor)。
如果设计师没有显式地声明一个构造函数，编译器会自动生成一个缺省的构造函数。这个缺省的构造函数相当于一个空函数。

比如上面代码中，\verb|ArrayList() {};| 实际上就是一个缺省的构造函数。它没有参数，也没有任何操作。
...
\begin{verbatim*}
	...
	t_ele *data = nullptr;		/**< 存放数据的内存区域. */  
\end{verbatim*}
这两句语句，实际上显式指定了成员变量的值。这是一种防止未定义的变量的推荐做法。

但作为数值，用户需要的初始化数值很可能是一个具体的序列，比如 \verb|34, 12, 52, 16, 12|.
